...
describe("AI Digest CLI", () => {
  ...
  it("should ignore files based on .aiignore", async () => {
    const { stdout } = await runCLI();
    expect(stdout).toContain("Files ignored by .aiignore:");
  }, 10000);
  ...
  it("should respect the --input flag", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ai-code-ingest-test-'));
