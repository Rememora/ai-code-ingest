...
async function readIgnoreFile(inputDir: string, filename: string = '.aiignore'): Promise<string[]> {
  try {
...
async function aggregateFiles(inputDir: string, outputFile: string, useDefaultIgnores: boolean, removeWhitespaceFlag: boolean, showOutputFiles: boolean): Promise<void> {
  try {
    ...
    if (customIgnoredCount > 0) {
      console.log(formatLog(`Files ignored by .aiignore: ${customIgnoredCount}`, '🚫'));
    }
    ...
    if (fileSizeInBytes > MAX_FILE_SIZE) {
      ...
      console.log(formatLog('Consider adding more files to .aiignore to reduce the output size.', '💡'));
    } else {
